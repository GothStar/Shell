...
open class Shell {
    ...
    public init(_ type: String = "/bin/sh", env: [String: String] = [:]) {
        self.lockQueue = DispatchQueue(label: "shell.process.lock.queue")
        self.type = type
        self.env = env
    }
    ...
    public func run(_ command: String) throws -> String {
        ...
        return try self.lockQueue.sync {
            guard process.terminationStatus == 0 else {
                ...
                throw ShellError.generic(Int(process.terminationStatus), message)
            }
            guard let output = String(data: outputData, encoding: .utf8) else {
                throw ShellError.outputData
            }
     ...
     */
    public func run(_ command: String, completion: @escaping ((String?, Error?) -> Void)) {
        let queue = DispatchQueue(label: "shell.process.concurrent.queue", attributes: .concurrent)
        queue.async {
            do {
